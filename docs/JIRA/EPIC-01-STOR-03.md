...
## Acceptance Criteria
- [x] `scraper/requirements.txt` includes `undetected-chromedriver>=3.5` and `selenium`.
- [x] `scraper/setup.sh` and `scraper/setup.ps1` create/activate venv and install requirements. Example commands are present in the scripts.
- [x] `scraper/run_manual_login.sh` and `scraper/run_manual_login.ps1` execute `python claude_scraper.py --login` and print clear instructions about manual interaction and that Chrome windows must not be closed.
- [x] `docs/README-SETUP.md` references these scripts and Chrome requirement (Research.md lines 174-179) [`docs/Research.md:174-179`].

## Dependencies
- EPIC-01-STOR-01

## Tasks
- [x] Create `scraper/requirements.txt` with `undetected-chromedriver>=3.5` and `selenium`. (0.25h)
- [x] Create `scraper/setup.sh` (Unix) with example content:
  - `python3 -m venv scraper-env && source scraper-env/bin/activate && pip install -r requirements.txt`
- [x] Create `scraper/setup.ps1` (Windows PowerShell) with example content:
  - `python -m venv scraper-env; .\scraper-env\Scripts\Activate.ps1; pip install -r requirements.txt`
- [x] Create `scraper/run_manual_login.sh` and `scraper/run_manual_login.ps1` which run:
...
- [x] Add `scraper/README.md` describing expected log files (`scraper/scraper.log`), session file location (`scraper/chrome-profile/session.json`), and the no-Chrome-window-closing constraint. (0.5h)
